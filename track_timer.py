    ...
    for box in currentPredictedBoxes:
        if get_intersect.box_line(box, startLine) != False:
            ...
            if not started and side == 'right':
                started = True
                startFrame = count
                cv2.putText(frame, 'Start', (200, 800), cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 255, 255), 4, cv2.LINE_AA)
                ...
                if args.mode == 'full':
                    ...
                    print("Total time is " + str(minutes) + ":" + str(round(seconds, 2)))
                finished = True
                cv2.putText(frame, 'Finish: ' + timeStr, (200, 800), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 255, 0), 4, cv2.LINE_AA)

    if started and count - startFrame < 90:
        cv2.putText(frame, 'Start', (200, 800), cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 255, 255), 4, cv2.LINE_AA)

    if finished and count - finishFrame < 90:
        cv2.putText(frame, 'Finish: ' + timeStr, (200, 800), cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 255, 255), 4, cv2.LINE_AA)
